...
export class DocumentReaderServer extends BaseMCPServer {
  ...
  private async readDocument(filePath: string, maxSize?: number): Promise<MCPToolResponse> {
    try {
      // Validate file
  ...
  private async analyzeDocumentMetadata(filePath: string): Promise<MCPToolResponse> {
    try {
      DocumentProcessor.validateFile(filePath);
  ...
  private async extractDocumentText(filePath: string, maxLength: number = 10000): Promise<MCPToolResponse> {
    try {
      DocumentProcessor.validateFile(filePath);
