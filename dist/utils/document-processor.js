...
export class DocumentProcessor {
    ...
    static getFormatName(extension) {
        const formatMap = {
            'pdf': 'PDF Document',
    ...
    static async getDocumentMetadata(filePath) {
        try {
    ...
    static async extractPDFMetadata(filePath) {
        ...
        if (info?.Subject)
            props.subject = info.Subject;
        if (info?.Keywords)
            props.keywords = info.Keywords;
        if (info?.Creator)
            props.creator = info.Creator;
        if (info?.Producer)
            props.producer = info.Producer;
        if (info?.CreationDate)
            props.creationDate = new Date(info.CreationDate).toISOString();
        if (info?.ModDate)
            props.modificationDate = new Date(info.ModDate).toISOString();
        return props;
    }
    static async extractDOCXMetadata(filePath) {
    ...
    static async extractDocumentContent(filePath) {
        ...
        try {
            ...
            if (metadata.documentProperties?.author)
                docMetadata.author = metadata.documentProperties.author;
            if (metadata.documentProperties?.creationDate)
                docMetadata.createdDate = metadata.documentProperties.creationDate;
            if (metadata.documentProperties?.modificationDate)
                docMetadata.modifiedDate = metadata.documentProperties.modificationDate;
            else
                docMetadata.modifiedDate = metadata.lastModified;
            if (metadata.documentProperties?.pageCount)
                docMetadata.pageCount = metadata.documentProperties.pageCount;
            if (metadata.documentProperties?.keywords)
                docMetadata.keywords = metadata.documentProperties.keywords;
            if (metadata.documentProperties?.subject)
                docMetadata.subject = metadata.documentProperties.subject;
    ...
    static async extractRTFContent(filePath) {
    ...
    static async checkFileSize(filePath, maxSize = 50 * 1024 * 1024) {
        try {
            const stats = await fs.stat(filePath);
